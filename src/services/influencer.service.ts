...
const createInfluencer = async function ({
  name,
  referralCode,
}: InfluencerParams) {
  try {
...
const updateInfluencer = async function (
  id: string,
  updates: Partial<InfluencerParams>
) {
  try {
    const influencer = await Influencer.findByIdAndUpdate(id, updates, {
      new: true,
    });
    if (!influencer) {
      throw new Error("Influencer not found.");
