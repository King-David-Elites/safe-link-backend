...
describe('OTP Controller Tests', () => {
  ...
  describe('generateOtp', () => {
    ...
    it('should return 400 if user is already verified', async () => {
      (userService.getById as jest.Mock).mockResolvedValue({ isVerified: true });

      await otpController.generateOtp(mockReq, mockRes, mockNext);

      expect(mockRes.status).toHaveBeenCalledWith(400);
      expect(mockRes.json).toHaveBeenCalledWith({ message: 'User is already verified' });
    });

    it('should return 200 and OTP if OTP is generated successfully', async () => {
      (userService.getById as jest.Mock).mockResolvedValue({ isVerified: false, phoneNumber: '+2349128062479' });
      (otpService.generateOtp as jest.Mock).mockResolvedValue('mockOtp');

      await otpController.generateOtp(mockReq, mockRes, mockNext);

      expect(mockRes.status).toHaveBeenCalledWith(200);
      expect(mockRes.json).toHaveBeenCalledWith({ message: 'OTP sent successfully', otp: 'mockOtp' });
    });
  });
  ...
  describe('verifyOtp', () => {
    ...
    it('should return 200 if OTP is valid and user is verified successfully', async () => {
      ...
      expect(mockRes.status).toHaveBeenCalledWith(200);
      expect(mockRes.json).toHaveBeenCalledWith({ message: 'User verified successfully' });
    });
  });
  ...
  describe('resendOtp', () => {
    ...
    it('should return 200 and OTP if OTP is resent successfully', async () => {
      (userService.getById as jest.Mock).mockResolvedValue({ isVerified: false, phoneNumber: '+2348161327903' });
      (otpService.generateOtp as jest.Mock).mockResolvedValue('mockOtp');

      await otpController.resendOtp(mockReq, mockRes, mockNext);

      expect(mockRes.status).toHaveBeenCalledWith(200);
      expect(mockRes.json).toHaveBeenCalledWith({ message: 'OTP resent successfully', otp: 'mockOtp' });
    });
  });
});
